...
%addpath(fname);
fname1 = 'E:/d/Dropbox/';%set the home folder
%fname1 = 'C:\Users\bailab\Dropbox\';
fnames = [fname1,'Pacbio_analysis\all_final4\'];
mkdir([fnames, 'nuc_prediction']);
fname = [fname1,'Pacbio_analysis\all_final\'];
list = [{'IR2C'};{'NR3C'};{'OR3C'};{'TR'};{'WTall'};{'WR2C'};{'17R'};{'N1R3'};{'N3R3'};{'O2R3'};{'O3R3'};{'NR3Cn'};{'OR3Cn'};{'T2RCn'};{'TRall'}];
motifs = load([fname1, 'Pacbio_analysis/ref/motif_pos_v2.mat']);%read motif positions
mkdir([fnames, 'nuc_prediction\tt']);%make folder to save nucleosome prediction data
...
for k = 15
    ...
    data = load([fname, 'norm_matrix/matrix_',name,'.mat']);
    tic;
    dbstop if error
    thub= [0,0];
    gcn = [19,17];
    ...
    for i = 144:145
        if k~=5
            met_all = data.data(i).C_T_sum_trim;
        else
            met_all = data.data.data(i).C_T_sum_trim;
        end
        mm = met_all(:,roi);
        mm(mm==-1)=0;
    ...
    parfor i = 1:169
        ...
        if k~=5
            matrix = data.data(i).C_T_sum_trim;%read the trimmed sequence matrix
        else
            matrix = data.data.data(i).C_T_sum_trim;%read the trimmed sequence matrix
        end
        ...
        gc_pos = readmatrix([fname1,'Pacbio_analysis\ref\GC_positions.xlsx'],'Sheet',i);
    ...
    toc;
    %output data
   save_data([fnames, 'nuc_prediction/tt/nuc_pred_pileup_all_TTxx_',name,'_',num2str(b),'_',num2str(linker),'.mat'],info4);
   save_data([fnames, 'nuc_prediction/tt/nuc_pred_TTxx_',name,'_',num2str(b),'_',num2str(linker),'.mat'],infom);
end%1-5
